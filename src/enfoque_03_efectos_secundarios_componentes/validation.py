...
Foco: Efectos Secundarios por Componentes.
...
Responsabilidades:
    ...
    - Validar columnas derivadas generadas por el pipeline de limpieza.
...
# Columnas derivadas que el pipeline de limpieza debe generar
...
def verificar_checksum(
    ...
    En la primera ejecución no hay checksum esperado, así que solo se registra
    el valor actual. En ejecuciones posteriores se puede pasar el checksum previo
    para detectar cambios en el archivo fuente.
    ...
    Returns:
        ...
        ``{"archivo": str, "md5": str, "tamanio_bytes": int, "coincide": bool | None, "estado": str}``
    """
    ruta = Path(ruta)
...
def validar_esquema(
    df: pd.DataFrame,
    esperado: dict[str, str] = ESQUEMA_ESPERADO,
) -> dict:
    """
    Verifica que el DataFrame tenga las columnas y tipos de datos esperados.

    Compara el esquema real del DataFrame contra el definido en `ESQUEMA_ESPERADO`.
    Detecta columnas faltantes, columnas extra inesperadas y tipos incorrectos.
    ...
    Returns:
        ...
        ``{"columnas_ok": list, "columnas_faltantes": list, "columnas_extra": list,
        "tipos_incorrectos": dict, "esquema_valido": bool}``
    """
...
def comparar_shapes(df_raw: pd.DataFrame, df_clean: pd.DataFrame) -> dict:
    ...
    Returns:
        ...
        ``{"filas_raw": int, "filas_clean": int, "filas_eliminadas": int,
        "porcentaje_eliminado": float, "columnas_raw": int, "columnas_clean": int,
        "columnas_añadidas": int, "columnas_nuevas": list}``
    """
    filas_raw = len(df_raw)
    filas_clean = len(df_clean)
    filas_eliminadas = filas_raw - filas_clean
    pct_eliminado = round((filas_eliminadas / filas_raw) * 100, 2) if filas_raw > 0 else 0.0
...
# ---------------------------------------------------------------------------
# Verificación 4: Columnas derivadas del DataFrame limpio
# ---------------------------------------------------------------------------
...
def validar_columnas_clean(df_clean: pd.DataFrame) -> dict:
    ...
    de componentes, efectos, conteos y anomalías definidas en
    `COLUMNAS_CLEAN_ESPERADAS`.

    Args:
        df_clean: DataFrame limpio a validar.

    Returns:
        Diccionario con columnas presentes, faltantes y estado de validez.
    """
...
def exportar_reporte(resultados: dict, nombre: str = "reporte_integridad_e03") -> Path:
    """
    Exporta el reporte de validación como archivo JSON en outputs/reports/.

    El formato JSON permite que el reporte sea legible por humanos y también
    procesable automáticamente en futuras auditorías del pipeline.
...
def run_validation_pipeline(
    ...
    print("=" * 55)
    print("INICIO DEL PIPELINE DE VALIDACIÓN  [Enfoque 3]")
    print("=" * 55)
    ...
    print("=" * 55)
    print("VALIDACIÓN COMPLETA  [Enfoque 3]")
    print("=" * 55)
